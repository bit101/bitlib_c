...
double bl_perlin_1(double x) { return bl_perlin_3(x, 0, 0); }

double bl_perlin_2(double x, double y) { return bl_perlin_3(x, y, 0); }
...
double grad3(unsigned hash, double x, double y, double z) {
  ...
  double v =
      Q(h < 4, y, Q(h == 12 || h == 14, x, z)); // Fix repeats at h = 12 to 15
...
double bl_simplex_3(double x, double y, double z) {
  ...
  double x1 =
      x0 - (double)i1 + G3; // Offsets for second corner in (x,y,z) coords
  double y1 = y0 - (double)j1 + G3;
  double z1 = z0 - (double)k1 + G3;
  double x2 =
      x0 - (double)i2 + 2 * G3; // Offsets for third corner in (x,y,z) coords
  ...
  if (t1 < 0) {
    ...
    t1 *= t1;
    n1 = t1 * t1 *
         grad3(perm[ii + i1 + (int)perm[jj + j1 + (int)perm[kk + k1]]], x1, y1,
               z1);
  }
  ...
  if (t2 < 0) {
    ...
    t2 *= t2;
    n2 = t2 * t2 *
         grad3(perm[ii + i2 + (int)perm[jj + j2 + (int)perm[kk + k2]]], x2, y2,
               z2);
  }
  ...
  if (t3 < 0) {
    ...
    n3 =
        t3 * t3 *
        grad3(perm[ii + 1 + (int)perm[jj + 1 + (int)perm[kk + 1]]], x3, y3, z3);
  }
